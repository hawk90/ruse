...
//! a typed query, a case-insensitive-substring filtered match list over items, a selection, and a
//! transient keymap. The command palette, line picker, and buffer picker are all `Picker<T>` over
//! different payloads (`Command` / byte offset / `DocumentId`) and different accept-actions — the caller
//! performs the payload-specific action when a row is accepted.
...
impl<T> Picker<T> {
    ...
    /// Recompute `matches` from the query (case-insensitive substring on each item's `search` text).
    fn refilter(&mut self) {
        ...
        self.matches = self
            ...
            .iter()
            .enumerate()
            .filter(|(_, it)| q.is_empty() || it.search.to_lowercase().contains(&q))
            .map(|(i, _)| i)
            .collect();
...
mod picker_tests {
    ...
    #[test]
    fn filters_by_substring_and_tracks_selection() {
        let mut p = Picker::new(items());
        assert_eq!(p.rows().len(), 3, "empty query keeps all");
        assert_eq!(p.selected(), Some(&0));

        // Type "beta" → two matches (beta, gamma beta); selection clamps to the first.
        for c in "beta".chars() {
            assert!(matches!(
