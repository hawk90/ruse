...
| 2 | `usable-vim-tui` | mvp | F-003, F-004, F-006, F-007, F-008, F-009, F-010 | A real editor: key → command → transaction → document → render, on the proven core. Vim profile, command palette, buffers/windows, file open/save + recovery, search/substitute, terminal detection. |
