...
def _resolve_profile(name: str) -> tuple[list[str], list[str]]:
    ...
    import yaml
    data = yaml.safe_load(open(repo.path("spec/context-profiles.yaml"))) or {}
...
def build(issue: str | None, roots: list[str], depth: int,
    ...
    roots = [r for r in roots if m.has(r)]
    ...
    # ---- render markdown ----
    L = ["<!-- GENERATED by `ruse context build` — do not hand-edit; rebuild instead. -->"]
